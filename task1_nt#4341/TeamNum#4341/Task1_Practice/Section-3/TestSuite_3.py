...
if __name__ == "__main__":
    fo = open("section3.txt","rb")
    string = fo.read()
    array_1 = parseData(string)
    array_2 = []
    for m in range(10):
        array_2.append(main("maze0" + str(m) + ".jpg"))
    flag = 0
    print "================================================="
    for m in range(10):
        try:
            print "Testing solution for maze0" + str(m) + ".jpg"
            assert array_1[m] == array_2[m]
            print "Solution passed for maze0" + str(m) + ".jpg\n"
    ...
    hasher = hashlib.md5()
    with open('TestSuite_3.py', 'rb') as afile:
         buf = afile.read()
         hasher.update(buf)
    fo = open("hash.txt", "wb")
